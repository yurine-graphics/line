...
class Line {
  ...
  render() {
    ...
    var [left, bottom, stepX, stepY] = self.renderBg(context, padding, width, height, gridWidth, min, lineHeight, fontSize, xNum, yNum, stepV, xLineNum, yLineNum);
    if(yNum == 1) {
      stepV = stepY >> 1;
    }
  ...
  renderBg(context, padding, width, height, gridWidth, min, lineHeight, fontSize, xNum, yNum, stepV, xLineNum, yLineNum) {
  ...
  renderY(context, padding, width, height, yNum, min, stepY, yLineNum, stepY2, fontSize, stepV, bottom) {
    var left = 0;
    var x = padding[3];
  ...
  renderFg(context, height, lineHeight, lineWidth, breakLineWidth, left, bottom, top, right, stepX, stepY, stepV, min, xLineNum, yLineNum) {
    ...
    coords.forEach(function(item, i) {
      ...
      self.renderLine(context, item, i, lineWidth, breakLineWidth, lineHeight, color, breakColor, breakDash, style, height - bottom, top, right, xLineNum, yLineNum);
    });
  }
  ...
  renderLine(context, coords, index, lineWidth, breakLineWidth, lineHeight, color, breakColor, breakDash, style, y, y0, right, xLineNum, yLineNum) {
    var self = this;
    switch(style) {
      case 'curve':
        self.renderCurve(context, coords, index, y, y0, right, color, lineWidth, breakLineWidth, breakColor, breakDash, xLineNum, yLineNum);
        break;
      default:
        self.renderStraight(context, coords, index, y, right, color, lineWidth, breakLineWidth, breakColor, breakDash);
        break;
    }
    if(self.option.discRadio) {
      var discRadio = parseInt(self.option.discRadio) || 1;
  ...
  renderCurve(context, coords, index, y, y0, x, color, lineWidth, breakLineWidth, breakColor, breakDash, xLineNum, yLineNum) {
    if(coords.length) {
      var clone = [];
      coords.forEach(function(item) {
        if(item === null || item === undefined) {
      ...
      for(++i; i < len - 1; i++) {
        if(coords[i]) {
          ...
          if(isPrevBreak) {
            ...
            var left = ctrols[i - 2 - count];
            var right = ctrols[i - 1 - count];
            if(right) {
              context.bezierCurveTo(left[2], left[3], right[0], right[1], coords[i][0], coords[i][1]);
            }
            else {
              context.quadraticCurveTo(left[2], left[3], coords[i][0], coords[i][1]);
            }
          }
          isPrevBreak = false;
        }
  ...
  renderStraight(context, coords, index, y, x, color, lineWidth, breakLineWidth, breakColor, breakDash) {
    context.beginPath();
    var fill = this.option.areaColors[index];
    if(fill == 'transparent') {
      fill = null;
